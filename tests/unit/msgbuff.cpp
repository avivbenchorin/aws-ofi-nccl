/*
 * Copyright (c) 2023 Amazon.com, Inc. or its affiliates. All rights reserved.
 */

#include <stdio.h>

#include "config.h"

#include "nccl_ofi_msgbuff.h"

#include "test-common.h"

int main(int argc, char *argv[])
{
	ofi_log_function = logger;
	const uint16_t max_inprogress = 4;
	const uint16_t num_msg_seq_num_bits = 4;
	const uint16_t field_size = 1 << num_msg_seq_num_bits;
	uint16_t *result;

	uint16_t *buff_store = (uint16_t *)calloc(max_inprogress, sizeof(uint16_t));
	if (!buff_store) {
		NCCL_OFI_WARN("Memory allocation failed");
		return 1;
	}

	for (uint16_t i = 0; i < max_inprogress; ++i) {
		buff_store[i] = i;
	}

	nccl_ofi_msgbuff_t *msgbuff = NULL;
	msgbuff = new nccl_ofi_msgbuff_t(max_inprogress, num_msg_seq_num_bits);
	if (msgbuff == NULL) {
		NCCL_OFI_WARN("nccl_ofi_msgbuff_init failed");
		return 1;
	}

	nccl_ofi_msgbuff_t::status stat;
	nccl_ofi_msgbuff_t::elemtype type = nccl_ofi_msgbuff_t::elemtype::REQ;
	uint16_t msg_seq_num = 0;
	uint16_t last_completed = (1 << num_msg_seq_num_bits) - 1;

	for (int rounds = 0; rounds < 4; rounds++) {
		/** Test insert new **/
		for (uint16_t i = 0; i < max_inprogress; ++i) {
			if (msgbuff->insert((msg_seq_num + i) % field_size, &buff_store[i], type,
						    &stat) != nccl_ofi_msgbuff_t::result::SUCCESS) {
				NCCL_OFI_WARN("nccl_ofi_msgbuff_insert failed when non-full");
				return 1;
			}
		}

		if (msgbuff->insert((msg_seq_num + max_inprogress) % field_size, NULL, type, &stat) !=
		nccl_ofi_msgbuff_t::result::INVALID_IDX ||
		    stat != nccl_ofi_msgbuff_t::status::UNAVAILABLE) {
			NCCL_OFI_WARN("nccl_ofi_msgbuff_t::insert did not return unavailable when full");
			return 1;
		}

		if (msgbuff->insert((msg_seq_num + max_inprogress - 1) % field_size, NULL, type,
					    &stat) != nccl_ofi_msgbuff_t::result::INVALID_IDX ||
		    stat != nccl_ofi_msgbuff_t::status::INPROGRESS) {
			NCCL_OFI_WARN("nccl_ofi_msgbuff_t::insert did not return inprogress on duplicate insert");
			return 1;
		}

		/** Test retrieve **/
		for (uint16_t i = 0; i < max_inprogress; ++i) {
			if (msgbuff->retrieve((msg_seq_num + i) % field_size, (void **)&result, &type,
						      &stat) != nccl_ofi_msgbuff_t::result::SUCCESS) {
				NCCL_OFI_WARN("nccl_ofi_msgbuff_t::retrieve failed on valid index");
				return 1;
			}
			if (*result != buff_store[i]) {
				NCCL_OFI_WARN("nccl_ofi_msgbuff_t::retrieve returned incorrect value");
				return 1;
			}
		}

		if (msgbuff->retrieve((msg_seq_num + max_inprogress) % field_size, (void **)&result,
					      &type, &stat) != nccl_ofi_msgbuff_t::result::INVALID_IDX ||
		    stat != nccl_ofi_msgbuff_t::status::NOTSTARTED) {
			NCCL_OFI_WARN("nccl_ofi_msgbuff_t::retrieve did not return notstarted");
			return 1;
		}

		if (msgbuff->retrieve(last_completed, (void **)&result, &type, &stat) !=
			    nccl_ofi_msgbuff_t::result::INVALID_IDX ||
		    stat != nccl_ofi_msgbuff_t::status::COMPLETED) {
			NCCL_OFI_WARN("nccl_ofi_msgbuff_t::retrieve did not return completed");
			return 1;
		}

		/** Test complete **/
		for (uint16_t i = 0; i < max_inprogress; ++i) {
			if (msgbuff->complete((msg_seq_num + i) % field_size, &stat) !=
			    nccl_ofi_msgbuff_t::result::SUCCESS) {
				NCCL_OFI_WARN("nccl_ofi_msgbuff_t::complete failed");
				return 1;
			}
		}

		if (msgbuff->complete((msg_seq_num + max_inprogress) % field_size, &stat) !=
			    nccl_ofi_msgbuff_t::result::INVALID_IDX ||
		    stat != nccl_ofi_msgbuff_t::status::NOTSTARTED) {
			NCCL_OFI_WARN("nccl_ofi_msgbuff_t::complete did not return notstarted");
			return 1;
		}

		if (msgbuff->complete(msg_seq_num, &stat) != nccl_ofi_msgbuff_t::result::INVALID_IDX ||
		    stat != nccl_ofi_msgbuff_t::status::COMPLETED) {
			NCCL_OFI_WARN("nccl_ofi_msgbuff_complete did not return completed");
			return 1;
		}

		last_completed = (msg_seq_num + max_inprogress - 1) % field_size;
		msg_seq_num = (msg_seq_num + max_inprogress) % field_size;
	}
	delete msgbuff;
	// if (!nccl_ofi_msgbuff_destroy(msgbuff)) {
	// 	NCCL_OFI_WARN("nccl_ofi_msgbuff_destroy failed");
	// 	return 1;
	// }

	free(buff_store);

	/** Success! **/
	return 0;
}
