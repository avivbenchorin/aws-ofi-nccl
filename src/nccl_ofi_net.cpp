...
int nccl_net_ofi_create_plugin(nccl_net_ofi_plugin_t **plugin_p)
{
	...
	plugin->isend_mutex_unlock_total = new timer_histogram<histogram_custom_binner<size_t> >("isend(), mutex unlock total", histogram_custom_binner<size_t>(total_bins), ovh_0);

	plugin->isend_mutex_unlock_send_progress_non_fi_write = new timer_histogram<histogram_custom_binner<size_t> >("isend(), mutex unlock total when reaching send_progress for non fi_writes", histogram_custom_binner<size_t>(total_bins), ovh_0);

	plugin->isend_mutex_unlock_send_progress_fi_write = new timer_histogram<histogram_custom_binner<size_t> >("isend(), mutex unlock total when reaching send_progress for fi_writes", histogram_custom_binner<size_t>(total_bins), ovh_0);

	plugin->isend_mutex_unlock_non_send_progress = new timer_histogram<histogram_custom_binner<size_t> >("isend(), mutex unlock total when not reaching send_progress", histogram_custom_binner<size_t>(total_bins), ovh_0);
#endif
